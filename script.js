...
let monthlyBudget = 0;
let currentAccountId = 'default-account'; // Track current account
let accounts = []; // Store multiple accounts

// Data storage - Initialize as empty arrays, will be populated via API
let expenses = [];
...
function initializeAccounts() {
    ...
    currentAccountId = localStorage.getItem('currentAccountId') || accounts[0].id;
...
function loadAccountData() {
    // Load account-specific data from localStorage
    const accountDataKey = `account-${currentAccountId}`;
    ...
    if (accountData) {
        try {
            ...
            if (currentAccount) {
                currentAccount.lastUsed = new Date().toISOString();
                localStorage.setItem('accounts', JSON.stringify(accounts));
            }
...
function logout() {
    ...
    try {
        ...
        // Reset UI elements
        document.getElementById('username').innerHTML = `Welcome to FinFlow`;
        ...
        document.getElementById('recentExpenseList').innerHTML = `
            <div class="empty-state">
                <i class="fas fa-receipt"></i>
                <p>No expenses yet</p>
        ...
        if (incomeExpenseSavingsChart) incomeExpenseSavingsChart.destroy();
        if (monthlyTrendChart) monthlyTrendChart.destroy();
        if (categoryChart) categoryChart.destroy();
        if (detailedCategoryChart) detailedCategoryChart.destroy();
        ...
        setTimeout(() => {
            toggleProfile();
            showNotification('Please set up your profile to continue', 'info');
        }, 1000);
...
function createSwitchAccountModal() {
    const modalHTML = `
        <div id="switchAccountModal" class="modal hidden">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="switch-account-form">
                        <div class="account-options">
                            <h4>Select Account Type</h4>
                            <div class="account-type-selector">
                                <button class="account-type-btn active" onclick="selectAccountType('new')">
                                    <i class="fas fa-plus-circle"></i>
                                    <span>Create New Account</span>
                                </button>
                                <button class="account-type-btn" onclick="selectAccountType('existing')">
                                    <i class="fas fa-user"></i>
                                    <span>Use Existing Account</span>
                                </button>
                            </div>
                            
                            <div id="newAccountForm" class="account-form-section">
                                <h4>Create New Account</h4>
                                <div class="form-group">
                                    <label for="newAccountName"><i class="fas fa-user"></i> Account Name</label>
                                    <input id="newAccountName" placeholder="e.g., Personal, Business, Family" />
                                </div>
                                <div class="form-group">
                                    <label for="newAccountType"><i class="fas fa-tag"></i> Account Type</label>
                            ...
                            <div id="existingAccountForm" class="account-form-section hidden">
                                <h4>Select Existing Account</h4>
                                <div id="existingAccountsList" class="accounts-list">
                                    <!-- Existing accounts will be populated here -->
                                </div>
                                <div class="form-group">
                                    <button class="btn-text" onclick="showImportAccountModal()">
                        ...
                        <div class="modal-actions">
                            <button class="btn-secondary" onclick="closeSwitchAccountModal()">Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="importAccountModal" class="modal hidden">
            <div class="modal-content">
                <div class="modal-header">
                    <h3><i class="fas fa-file-import"></i> Import Account</h3>
                    <button class="modal-close" onclick="closeImportAccountModal()">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <div class="import-form">
                        <div class="form-group">
                            <label for="importAccountName"><i class="fas fa-user"></i> Account Name</label>
                            <input id="importAccountName" placeholder="Enter a name for this account" />
                        </div>
                        
                        <div class="form-group">
                            <label for="importFile"><i class="fas fa-file-upload"></i> Import File</label>
                            ...
                            <p class="help-text">Export your account data from another device and upload the JSON file here.</p>
                        </div>
                        
                        <div class="modal-actions">
                            <button class="btn-secondary" onclick="closeImportAccountModal()">Cancel</button>
                            <button class="btn-primary" onclick="importAccount()">
                                <i class="fas fa-file-import"></i> Import Account
                            </button>
                        </div>
                    </div>
...
function selectAccountType(type) {
    const newAccountBtn = document.querySelector('.account-type-btn[onclick*="new"]');
    const existingAccountBtn = document.querySelector('.account-type-btn[onclick*="existing"]');
    const newAccountForm = document.getElementById('newAccountForm');
    const existingAccountForm = document.getElementById('existingAccountForm');
    
    if (type === 'new') {
        newAccountBtn.classList.add('active');
        existingAccountBtn.classList.remove('active');
        newAccountForm.classList.remove('hidden');
        existingAccountForm.classList.add('hidden');
    } else {
        newAccountBtn.classList.remove('active');
        existingAccountBtn.classList.add('active');
        newAccountForm.classList.add('hidden');
        existingAccountForm.classList.remove('hidden');
        updateExistingAccountsList();
    }
}

function updateExistingAccountsList() {
    const existingAccountsList = document.getElementById('existingAccountsList');
    ...
    accounts.forEach(account => {
        ...
        html += `
            <div class="account-item ${isCurrent ? 'current' : ''}">
                <div class="account-info">
                    <div class="account-icon">
                        <i class="fas fa-${getAccountTypeIcon(account.type)}"></i>
                    </div>
                    <div class="account-details">
                        <h4>${account.name}</h4>
                        <p class="account-meta">
                            <span class="account-type">${account.type}</span>
                            <span class="account-date">Last used: ${lastUsed}</span>
                        </p>
                    </div>
                </div>
                <div class="account-actions">
                    ...
                    ` : `
                        <button class="btn-text" onclick="switchToAccount('${account.id}')">
                            <i class="fas fa-sign-in-alt"></i> Switch
                        </button>
                        <button class="btn-icon" onclick="exportAccount('${account.id}')" title="Export">
    ...
    existingAccountsList.innerHTML = html;
}
...
function createNewAccount() {
    ...
    showNotification(`Account "${accountName}" created successfully`, 'success');
}
...
function switchToAccount(accountId) {
    ...
    if (account) {
        showNotification(`Switched to "${account.name}" account`, 'success');
    }
}
...
function deleteAccount(accountId) {
    ...
    // Remove account data from localStorage
    localStorage.removeItem(`account-${accountId}`);
    
    // If deleting current account, switch to another
    ...
    showNotification(`Account "${account.name}" deleted`, 'success');
}
...
function exportAccount(accountId) {
    ...
    // Get account data
    const accountDataKey = `account-${accountId}`;
    const accountData = localStorage.getItem(accountDataKey);
    
    if (!accountData) {
        showNotification('No data to export for this account', 'warning');
        return;
    }
    ...
    try {
        ...
        const exportFileDefaultName = `finflow-${account.name.toLowerCase().replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.json`;
        ...
        showNotification(`Account "${account.name}" exported successfully`, 'success');
        
    } catch (error) {
        console.error('Error exporting account:', error);
        showNotification('Error exporting account', 'error');
    }
}

function showImportAccountModal() {
...
function importAccount() {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!importedData.account || !importedData.data) {
            ...
            const newAccount = {
                id: 'account-' + Date.now(),
                name: accountName,
                type: importedData.account.type || 'personal',
                created: new Date().toISOString(),
                lastUsed: new Date().toISOString(),
                isDefault: false
            };
            ...
            // Save imported data
            const accountDataKey = `account-${newAccount.id}`;
            localStorage.setItem(accountDataKey, JSON.stringify(importedData.data));
            ...
            showNotification(`Account "${accountName}" imported successfully`, 'success');
            
        } catch (error) {
            console.error('Error importing account:', error);
            showNotification('Error importing account. Invalid file format.', 'error');
        }
    };
...
function updateExistingCategory() {
    ...
    updateExpenseCategories(oldCategoryName, categoryName);
...
Are you sure you want to delete this category?`)) {
    ...
    // Update expenses via API
    updateExpenseCategories(categoryName, 'Other');
    
    // Update local recurring expenses and bill reminders
    ...
    showNotification(`Category "${categoryName}" deleted. Expenses moved to "Other" category.`, 'success');
    updateAllDisplays();
    updateManageCategoriesModal();
}
...
// Add notification and account management styles
...
.switch-account-form .account-type-selector {
    display: flex;
    gap: 0.75rem;
    margin: 1rem 0;
}
